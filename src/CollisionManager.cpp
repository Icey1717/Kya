...
void CCollision::UpdateMatrix(edF32MATRIX4* param_2)
{
	...
	peVar2 = this->pMatrix;
	if (peVar2 != (edF32MATRIX4*)0x0) {
		IMPLEMENTATION_GUARD(
		edF32Matrix4MulF32Matrix4Hard(peVar2 + 1, peVar2, &this->transformMatrix);)
	}
	return;
}
...
uint CCollision::CheckCollisionsWithActors(CActor* pActor, edF32MATRIX4* m0)
{
	...
	if (local_110.entryCount != 0) {
		...
		if (0 < local_110.entryCount) {
			do {
				COLLISION_LOG(LogLevel::VeryVerbose, "CCollision::CheckCollisionsWithActors {} with: {}", pActor->name, local_110.entries[iVar5]->name);

				CCollision* pCollision = local_110.entries[iVar5]->pCollisionData;

				local_170.pOtherColObj = pCollision->pColObj;
				uVar3 = edObbTreeIntersectObbTree(&local_170, peVar1, pCollision->pObbTree);

				COLLISION_LOG(LogLevel::VeryVerbose, "CCollision::CheckCollisionsWithActors {} with: {} result: {}", pActor->name, local_110.entries[iVar5]->name, uVar3);
...
uint CCollision::CheckCollisionsWithScenery(int param_2)
{
	...
	if (param_2 == 0) {
		...
		IMPLEMENTATION_GUARD(
		pObbTreeB = (edObbTREE_DYN*)((CScene::ptable.g_SectorManager_00451670)->baseSector).pManager100Obj;
...
uint CCollision::ResolveContacts(CActor* pActor, edF32VECTOR4* pVector, int param_4)
{
	short sVar1;
	CCollision* pCVar2;
	edF32QUAD4* peVar3;
	...
	if (bVar4) {
		bVar4 = 0.001f <= fabs(pVector->y);
	}
	if (!bVar4) {
		bVar4 = 0.001f <= fabs(pVector->x);
	}
	if (!bVar4) {
		bVar4 = 0.001f <= fabs(pVector->z);
	}
		...
		if (pColDbObj->field_0x78 < 0.0) {
			...
			if (pColDbObj->pOtherColObj == (edColOBJECT*)0x0) {
				...
				if (pReceiver != (CActor*)0x0) {
					...
					fVar20 = (pColDbObj->field_0x30).y;
					fVar18 = (pColDbObj->field_0x30).z;
					fVar19 = (pColDbObj->field_0x30).w;
					(pCVar2->field_0x90).x = (pColDbObj->field_0x30).x;
					(pCVar2->field_0x90).y = fVar20;
					(pCVar2->field_0x90).z = fVar18;
					(pCVar2->field_0x90).w = fVar19;
				}
				iVar14 = 1;)
			}
	...
	if ((this->flags_0x0 & 8) != 0) {
		...
		if (0 < iVar14) {
			if (((this->flags_0x0 & 0x20) != 0) && (iVar15 = 0, (this->flags_0x4 & 2) != 0)) {
				for (; iVar15 < 2; iVar15 = iVar15 + 1) {
					IMPLEMENTATION_GUARD_LOG(
					local_d0[iVar15 * 0x18 + 1] = 0.0;
					local_c0[iVar15 * 0x18 + 0xd] = 0;)
				}
			}

			for (iVar15 = 0; iVar15 < 2; iVar15 = iVar15 + 1) {
				peVar8 = local_120 + iVar15;
				*peVar8 = gF32Vector4Zero;

				for (iVar12 = 0; iVar12 < 3; iVar12 = iVar12 + 1) {

					ColHeapEntryB* pEntry = &stack0xfffffdb0[iVar15][iVar12];
					pEntry->field_0x0 = pEntry->field_0x0 + *peVar8;
				}
			}

			if (iVar16 != 0) {
				edF32VECTOR4 local_130;
				edF32Vector4SafeNormalize0Hard(&local_130, peVar8);
		...
		if (1e-12f <= local_e0.x * local_e0.x + local_e0.y * local_e0.y + local_e0.z * local_e0.z) {
			if ((this->flags_0x0 & 0x40) == 0) {
				*pVector = *pVector + local_e0;
			}
			else {
				edF32Vector4SafeNormalize0Hard(&local_150, pVector);
				fVar18 = (pVector->x + local_e0.x) * local_150.x + (pVector->y + local_e0.y) * local_150.y + (pVector->z + local_e0.z) * local_150.z;
				*pVector = local_150 * fVar18;
			}
...
uint CCollision::CheckCollisions_OBBTree(CActor* pActor, edF32MATRIX4* m0, edF32MATRIX4* param_4, edF32VECTOR4* v0,	CActorsTable* param_6, CActor** param_7, int param_8)
{
	int iVar1;
	uint uVar2;
	uint uVar3;
	...
	if (((this->flags_0x0 & 0x502) != 0) && (uVar2 = CheckCollisionsWithActors(pActor, m0), uVar2 != 0)) {
		if ((param_6 != (CActorsTable*)0x0) || (((this->flags_0x0 & 0x100) != 0 || ((this->flags_0x0 & 0x400) == 0)))) {
			if ((this->flags_0x0 & 0x400) == 0) {
				IMPLEMENTATION_GUARD(
				PreprocessActorContacts(0.0, this, pActor, param_6, param_7);)
			}
			else {
				IMPLEMENTATION_GUARD(
				PreprocessActorContacts(0.005, this, pActor, param_6, param_7);)
			}
		}
	...
	if (uVar2 == 0) {
		...
		uVar2 = ResolveContacts(pActor, v0, param_8);
	}
	return uVar2;
}
...
void CCollision::CheckCollisions_UpdateCollisionMatrix(CActor* pActor, edF32MATRIX4* pMatrix, CActorsTable* pActorsTable, CActor* param_5,	int param_6)
{
	...
	edF32Vector4SubHard(&local_160, &pMatrix->rowT, &(this->transformMatrix).rowT);
	...
	if ((this->flags_0x0 & 4) == 0) {
		...
		if ((this->pObbPrim == (edColPRIM_OBJECT*)0x0) || ((this->flags_0x0 & 0x10000) == 0)) {
			...
			IMPLEMENTATION_GUARD(
			iVar4 = GetPrimNumDeltaSubdivisions(this->pObbPrim, this->primType, &local_160);)
		}
		if (0x14 < iVar4) {
			iVar4 = 0x14;
		}
		float fStack356 = 1.0f / (float)iVar4;
		local_170 = local_160 * fStack356;
		local_180 = (this->transformMatrix).rowT;

		for (; iVar4 != 0; iVar4 = iVar4 + -1) {
			local_190 = local_170;
			pMatrix->rowT = local_180 + local_170;
			...
			if (bVar2) {
				...
				local_150.da = 0.0 - pMatrix->da;
				local_150.db = 0.0 - pMatrix->db;
				local_150.dc = 0.0 - pMatrix->dc;
				if ((this->flags_0x4 & 0x10) == 0) {
			...
			uVar5 = CheckCollisions_OBBTree(pActor, pMatrix, &local_150, &local_190, pActorsTable, &local_4, param_6);
			uVar9 = uVar9 | uVar5;
			local_180 = local_180 + local_190;
		}

		pMatrix->rowT = local_180;
		...
		IMPLEMENTATION_GUARD(
		CheckCollisions_RayIntersect(pActor, &(this->transformMatrix).rowT, &local_160, pActorsTable, param_6);)
		pMatrix->rowT = (this->transformMatrix).rowT + local_160;
	...
	if ((uVar3 != 0) && ((pActorsTable->entryCount != 0 || (this->field_0x20 != 0)))) {
		IMPLEMENTATION_GUARD(
	...
	peVar7 = this->pMatrix;
	if (peVar7 != (edF32MATRIX4*)0x0) {
		edF32Matrix4MulF32Matrix4Hard(peVar7 + 1, peVar7, &this->transformMatrix);
	}
