...
struct VS_INPUT_UNPROCESSED
{
	int2 st : TEXCOORD0;
	float q : TEXCOORD1;
	float _pad : TEXCOORD2;
    uint4 c : COLOR0;
	float3 p : POSITION0;
...
PS_OUTPUT ps_main(PS_INPUT input)
{
	...
	output.c0 = C / 255.0f;
	output.c1 = input.p.z; //alpha_blend;
...
VS_OUTPUT ProcessInput(VS_INPUT input)
{
    if (VS_BPPZ == 1) // 24
	{
