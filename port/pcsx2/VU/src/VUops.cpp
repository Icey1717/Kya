...
static __ri void _vuXITOP(VURegs* VU)
{
	...
	if (VU == &VU1 && THREAD_VU1)
		VU->VI[_It_].US[0] = 0; // vu1Thread.vifRegs.itop;
	else
		VU->VI[_It_].US[0] = VU->GetVifRegs().itop;
}

extern std::function<void(int)> gKickCallback;
...
static __ri void _vuXTOP(VURegs* VU)
{
	...
	if (VU == &VU1 && THREAD_VU1)
		VU->VI[_It_].US[0] = 0; // (u16)vu1Thread.vifRegs.top;
	else
