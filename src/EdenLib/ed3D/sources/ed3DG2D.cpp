...
ed_g2d_manager* ed3DG2DDuplicateMaterial(ed_hash_code* pHashCode, char* pData, ed_g2d_manager* pG2d)
{
	...
	pdVar6 = LOAD_SECTION_CAST(dummy_built_g2d_source*, pHashCode->pData);
	...
	(pNewG2d->hashCode).pData = STORE_SECTION(&pNewG2d->material);
	...
	peVar7 = LOAD_SECTION_CAST(ed_Chunck*, pdVar6->material.aLayers[0]);
	pNewG2d->material.aLayers[0] = STORE_SECTION(&pNewG2d->LAY);
	...
	pSrcTEX = LOAD_SECTION_CAST(ed_Chunck*, pLayerSrc->pTex);
	(pNewG2d->layer).pTex = STORE_SECTION(&pNewG2d->TEX);
	...
	if (pSrcTexture->pAnimSpeedNormalExtruder == 0x0) {
		...
		edF32VECTOR4* pSrcAnimExtruder = LOAD_SECTION_CAST(edF32VECTOR4*, pSrcTexture->pAnimSpeedNormalExtruder);
		edF32VECTOR4* pDstAnimExtruder = reinterpret_cast<edF32VECTOR4*>(pDstHashCodes);
		*pDstAnimExtruder = *pSrcAnimExtruder;
		(pNewG2d->texture).pAnimSpeedNormalExtruder = STORE_SECTION(pDstAnimExtruder);
