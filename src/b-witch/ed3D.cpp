...
inline void ed3DListAddNodeSorted(edLIST* pList, void* pData, byte type)
{
	edNODE_MANAGER* pNodeManager = (edNODE_MANAGER*)pList->pData;
	edNODE* pNode = pNodeManager->pNodeHead + pNodeManager->linkCount;
	pNode->pData = pData;

	pNode->header.typeField.type = type;
...
inline void ed3DLinkSpriteToList(edLIST* pList, ed_3d_sprite* pSprite)
{
	edNODE_MANAGER* pNodeManager = (edNODE_MANAGER*)pList->pData;
	edNODE* pNode = pNodeManager->pNodeHead + pNodeManager->linkCount;
	pNode->pData = pSprite;
...
inline void ed3DLinkMaterialToPrimList(ed_dma_material* pMaterial, int PrimType)
{
	ed3DListAddNodeSorted(&gPrim_List[PrimType][gCurRenderList], pMaterial, LIST_TYPE_MATERIAL);
...
{
	ed_Chunck* pLAY = LOAD_POINTER_CAST(ed_Chunck*, pMaterial->pMaterial->aLayers[index]);
	*ppLayer = reinterpret_cast<ed_g2d_layer*>(pLAY + 1);

	ed_Chunck* pTEX = LOAD_POINTER_CAST(ed_Chunck*, (*ppLayer)->pTex);
	ed_g2d_texture* pTexture = reinterpret_cast<ed_g2d_texture*>(pTEX + 1);
	ed_hash_code* pTextureHashCode = LOAD_POINTER_CAST(ed_hash_code*, pTexture->hashCode.pData);
...
void ed3DFlushTexReset(edLIST* pList)
{
	...
	if (pMVar1->pMaterial->nbLayers < 2) {
		...
		if ((peVar1 != (ed_g2d_bitmap*)0x0) && (peVar1->psm != 0)) {
			edPSX2Header* pHeader = ((edPSX2Header*)LOAD_POINTER(peVar1->pPSX2)) + gVRAMBufferDMA;
			pPkt = (edpkt_data*)LOAD_POINTER(pHeader->pPkt);
			edDmaFlushCache();
...
void ed3DFlushSendDMA3D(void)
{
	...
	if (gPrim_List_FlushTex[gCurFlushList].pPrev != gPrim_List_FlushTex + gCurFlushList) {
...
{
	g_PKTHeaderRef[0].cmdA = ED_VIF1_SET_TAG_REF(1, STORE_POINTER(&g_stVertexGIFHeader));
	g_PKTHeaderRef[0].cmdB = 0;
	g_PKTHeaderRef[1].cmdA = ED_VIF1_SET_TAG_REF(2, STORE_POINTER(g_stVertexSTHeader));
	g_PKTHeaderRef[1].cmdB = 0;
	g_PKTHeaderRef[2].cmdA = ED_VIF1_SET_TAG_REF(1, STORE_POINTER(&g_stVertexRGBAHeader));
	g_PKTHeaderRef[2].cmdB = 0;
	g_PKTHeaderRef[3].cmdA = ED_VIF1_SET_TAG_REF(1, STORE_POINTER(&g_stVertexXYZHeader));
	g_PKTHeaderRef[3].cmdB = 0;
	g_PKTHeaderRef[4].cmdA = ED_VIF1_SET_TAG_REF(1, STORE_POINTER(&g_stNormalHeader));
	g_PKTHeaderRef[4].cmdB = 0;
	g_PKTHeaderRef[5].cmdA = ED_VIF1_SET_TAG_REF(1, STORE_POINTER(&g_stVertexOptionFlagHeader));
	g_PKTHeaderRef[5].cmdB = 0;
	g_PKTHeaderRef[6].cmdA = ED_VIF1_SET_TAG_REF(1, STORE_POINTER(&g_stVertexMultiSTHeader));
	g_PKTHeaderRef[6].cmdB = 0;
	g_PKTHeaderRef[7].cmdA = ED_VIF1_SET_TAG_REF(1, STORE_POINTER(&g_stVertexXYZKeepADCHeader));
...
{
	g_PKTSpriteHeaderRef[SPRITE_GIF_HEADER_INDEX].cmdA = ED_VIF1_SET_TAG_REF(1, STORE_POINTER(&g_stSpriteVertexGIFHeader));
	g_PKTSpriteHeaderRef[SPRITE_GIF_HEADER_INDEX].cmdB = 0;

	g_PKTSpriteHeaderRef[SPRITE_ST_HEADER_INDEX].cmdA = ED_VIF1_SET_TAG_REF(2, STORE_POINTER(g_stSpriteVertexSTHeader));
	g_PKTSpriteHeaderRef[SPRITE_ST_HEADER_INDEX].cmdB = 0;

	g_PKTSpriteHeaderRef[SPRITE_RGBA_HEADER_INDEX].cmdA = ED_VIF1_SET_TAG_REF(1, STORE_POINTER(&g_stSpriteVertexRGBAHeader));
	g_PKTSpriteHeaderRef[SPRITE_RGBA_HEADER_INDEX].cmdB = 0;

	g_PKTSpriteHeaderRef[SPRITE_XYZ_HEADER_INDEX].cmdA = ED_VIF1_SET_TAG_REF(1, STORE_POINTER(&g_stSpriteVertexXYZHeader));
	g_PKTSpriteHeaderRef[SPRITE_XYZ_HEADER_INDEX].cmdB = 0;

	g_PKTSpriteHeaderRef[SPRITE_WIDTH_HEIGHT_HEADER_INDEX].cmdA = ED_VIF1_SET_TAG_REF(2, STORE_POINTER(g_stSpriteWidthHeightHeader));
...
void ed3DDMAInit(uint countA, int countB)
{
	...
	g_PKTShapeHeaderRef[0].cmdA = ED_VIF1_SET_TAG_REF(1, STORE_POINTER(&g_stShapeVertexXYZHeader));
	g_PKTShapeHeaderRef[1].cmdA = ED_VIF1_SET_TAG_REF(2, STORE_POINTER(g_stShapeVertexSTHeader));
	g_PKTShapeHeaderRef[2].cmdA = ED_VIF1_SET_TAG_REF(1, STORE_POINTER(&g_stShapeVertexRGBAHeader));
	pcVar2 = ed3D_Allocator_00449248.end;
...
{
	ed_dma_matrix* pMatrixNode;

	pMatrixNode = DmaMatrixBufferCurrent;

	// Move the buffer forward.
	DmaMatrixBufferCurrent = DmaMatrixBufferCurrent + 1;

	pMatrixNode->pObjToWorld = pRenderInfo->pSharedMeshTransform;
	pMatrixNode->pHierarchy = pHierarchy;
	pMatrixNode->normalScale = pRenderInfo->biggerScale;

	// Link the new node to the previous one.
	pMatrixNode->pNext = pMatrixNode;
	pMatrixNode->pPrev = pMatrixNode;
	pMatrixNode->pData = gNodeDmaStrip;
	pMatrixNode->field_0x10 = 0;
	pMatrixNode->nodeCount = 0;
	pMatrixNode->header.mode = 0;

	return pMatrixNode;
}

ed_dma_material* ed3DListCreateDmaMaterialNode(float z, ed_g2d_material* pMaterial)
{
	ed_dma_material* pMaterialNode;

	pMaterialNode = DmaMaterialBufferCurrent;

	// Move the buffer forward.
	DmaMaterialBufferCurrent = DmaMaterialBufferCurrent + 1;

	pMaterialNode->pMaterial = pMaterial;
	pMaterialNode->field_0x4 = z;
	pMaterialNode->flags = pMaterialNode->flags & 0xfffffffe;

	// Link the new node to the previous one.
	pMaterialNode->list.pNext = &pMaterialNode->list;
	pMaterialNode->list.pPrev = &pMaterialNode->list;
	pMaterialNode->list.pData = gNodeDmaStrip;
	pMaterialNode->list.field_0x10 = 0;
	pMaterialNode->list.nodeCount = 0;
	pMaterialNode->list.header.mode = 0;
...
ed_g2d_bitmap* ed3DGetG2DBitmap(ed_g2d_material* pMaterial, int index)
{
	...
	if (pMaterial != (ed_g2d_material*)0x0) {
		...
		if ((((pMaterial->nbLayers == 0) || (iVar1 = pMaterial->aLayers[index], iVar1 == -0x10)) || (iVar1 == -0x10)) ||
			(pLAY = LOAD_POINTER_CAST(ed_Chunck*, iVar1), pLayer = reinterpret_cast<ed_g2d_layer*>(pLAY + 1), 
				((pLayer->bHasTexture == 0 || (pLayer->pTex == -0x10)) || (pTEX = LOAD_POINTER_CAST(ed_Chunck*, pLayer->pTex), pTexture = reinterpret_cast<ed_g2d_texture*>(pTEX + 1),
					iVar1 = pTexture->hashCode.pData, iVar1 == 0)))) {
			pBitmap = (ed_g2d_bitmap*)0x0;
		}
		else {
			ed_hash_code* pHash = LOAD_POINTER_CAST(ed_hash_code*, iVar1);
			pBitmap = reinterpret_cast<ed_g2d_bitmap*>(LOAD_POINTER_CAST(ed_Chunck*, pHash->pData) + 1);
		}
	}
...
ed_g2d_bitmap* ed3DGetG2DBitmap(ed_dma_material* pDMA_Material, int index)
{
	...
	if ((pMaterial == (ed_g2d_material*)0x0) || (pMaterial->nbLayers == 0)) {
		...
		pBitmap = ed3DGetG2DBitmap(pMaterial, index);
	}
...

{
	...
	if (pG2dMaterial != (ed_g2d_material*)0x0) {
		ed_Chunck* pLAY = LOAD_POINTER_CAST(ed_Chunck*, pG2dMaterial->aLayers[index]);
		...
		if ((((pG2dMaterial->nbLayers == 0) || (pLayer == (ed_g2d_layer*)0x0)) || (pLayer == (ed_g2d_layer*)0x0)) ||
			(((pLayer->bHasTexture == 0 || (pTexture == (ed_g2d_texture*)0x0)) ||
				((pTexture->bHasPalette <= pLayer->paletteId) ||
					(pPaletteHash = LOAD_POINTER_CAST(ed_hash_code*, pHashCode[pLayer->paletteId].pData), pPaletteHash == (ed_hash_code*)0x0))))) {
			pBitmap = (ed_g2d_bitmap*)0x0;
		}
		else {
			ed_Chunck* pPA32 = LOAD_POINTER_CAST(ed_Chunck*, pPaletteHash->pData);
...
void ed3DShadowResetPrimList(void)
{
	...
	pMeshCurrrent = ed3DListCreateDmaMaterialNode(0.0f, &gMaterial_Render_Zbuffer_Only);
	...
	pMeshCurrrent = ed3DListCreateDmaMaterialNode(0.0f, &gMaterial_Render_Zbuffer_Only_Cluster);
...
{
	g_VifRefPktCur->cmdA = ED_VIF1_SET_TAG_REF(1, STORE_POINTER(&g_stMatrixHeader));
...
void ed3DSceneSortClusters(ed_3D_Scene* pScene)
{
	...
	if (pPrevNode != pCurNode) {
		...
		for (; pPrevNode != pCurNode; pPrevNode = pPrevNode->pPrev) {
		...
		while (bListNotSorted) {
			...
			for (peVar1 = pPrevNode->pPrev; peVar1 != pCurNode; peVar1 = peVar1->pPrev) {
...
edpkt_data* ed3DPKTAddMatrixPacket(edpkt_data* pPkt, ed_dma_matrix* pDmaMatrix)
{
	...
	while (true) {
		...
		if (prevNodeType == LIST_TYPE_SPRITE) {
			ed_3d_sprite* pSprite = reinterpret_cast<ed_3d_sprite*>(pPrevNode->pData);
			if (((pSprite->pRenderFrame30 & 0x280) == 0) && (pDmaMatrix->pHierarchy != &ed_3d_hierarchy_0048cad0)) {
				bVar11 = 1;
			}
		}
		else {
	...
	*g_pCurFlareMtx = (edF32MATRIX4*)(reinterpret_cast<char*>(pDmaMatrix->pHierarchy->pMatrixPkt) + 0xa0);
	...
	pPkt->cmdA = ED_VIF1_SET_TAG_REF(0x1c, STORE_POINTER(pDmaMatrix->pHierarchy->pMatrixPkt) & 0xfffffffU);
	pPkt->asU32[2] = SCE_VIF1_SET_NOP(0);
	pPkt->asU32[3] = SCE_VIF1_SET_UNPACK(0x0006, 0x1c, UNPACK_V4_32, 0);
...
void DebugDumpStripFlagInfo(uint flags)
{
	...
	if ((flags & RENDER_STRIP_FLAG_ANIM_ST) != 0) {
...
edpkt_data* ed3DFlushStripInit(edpkt_data* pPkt, edNODE* pNode, ulong mode)
{
	...
	if ((uVar2 & 2) != 0) {
		if ((p3dStrip->flags & 0x2000000) == 0) {
			pNextPkt->cmdA = ED_VIF1_SET_TAG_REF(1, STORE_POINTER(&g_stGifTAG_FAN_Texture_NoFog));
			pNextPkt->asU32[2] = SCE_VIF1_SET_NOP(0);
			pNextPkt->asU32[3] = SCE_VIF1_SET_UNPACK(0x1, 0x1, UNPACK_V4_32_MASKED, 0);
			pNextPkt = pPkt + 7;
		}
		else {
			pNextPkt->cmdA = ED_VIF1_SET_TAG_REF(1, STORE_POINTER(&g_stGifTAG_FAN_Gouraud_NoFog));
			pNextPkt->asU32[2] = SCE_VIF1_SET_NOP(0);
			pNextPkt->asU32[3] = SCE_VIF1_SET_UNPACK(0x1, 0x1, UNPACK_V4_32_MASKED, 0);
			pNextPkt = pPkt + 7;
		}
	...
	if (((p3dStrip->flags & 0x10000) != 0) && (gpCurHierarchy != (ed_3d_hierarchy*)0x0)) {
		...
		VU1Emu::UpdateMemory(pPkt, pNextPkt);
#endif

		if (gBackupPKT != (edpkt_data*)0x0) {
			uint pktLen = PKT_SIZE(pTempPkt, gStartPKT_SPR);
			edDmaSync(SHELLDMA_CHANNEL_VIF0);
			...
			pNextPkt = SCRATCHPAD_ADDRESS(0x70000800);
			gStartPKT_SPR = pTempPkt;
		}
		...
		{
			pNextPkt->cmdA = ED_VIF1_SET_TAG_CNT(4);
			pNextPkt->asU32[2] = SCE_VIF1_SET_NOP(0);
			pNextPkt->asU32[3] = SCE_VIF1_SET_UNPACK(animMatrixDestAddr, 0x04, UNPACK_V4_32, 0);
			edF32Matrix4CopyHard(((edF32MATRIX4*)(pNextPkt + 1)), (pAnimMatrices + *pAnimIndexes));
		...
		if (gBackupPKT != (edpkt_data*)0x0) {
			edDmaSync(SHELLDMA_CHANNEL_VIF0);
			uint pktLen = PKT_SIZE(pNextPkt, SCRATCHPAD_ADDRESS(0x70000800));
	...
	if (((gpCurHierarchy == (ed_3d_hierarchy*)0x0) || (gpCurHierarchy == (ed_3d_hierarchy*)0x0000ffff))
		...
		if ((bDobfc$1270 == 0) && ((gpCurHierarchy->flags_0x9e & 2) == 0)) {
			p3dStrip->flags = p3dStrip->flags & 0xfffffff7;
		}
		else {
...
void ed3DFlushStripMultiTexture(edNODE* pNode, ed_g2d_material* pMaterial)
{
	...
	pCurPkt = g_VifRefPktCur;
	uVar25 = 0;

	materialNbLayers = pMaterial->nbLayers;
	pStrip = reinterpret_cast<ed_3d_strip*>(pNode->pData);

	stripMeshCountA = pStrip->meshCount;

	stripMeshCountB = (uint)stripMeshCountA;
	piVar5 = LOAD_POINTER_CAST(int*, pStrip->pSTBuf);
	iVar20 = 9;

	if ((pStrip->flags & 0x8000000) == 0) {
		iVar20 = 7;
	}
	...
	bVar1 = gFushListCounter != 0xe;
	mode = (ulong)materialNbLayers;
	if (!bVar1) {
		mode = 1;
	}
	...
	if (((pStrip->flags & 0x10000) == 0) || (gpCurHierarchy == (ed_3d_hierarchy*)0x0)) {
		if (((pStrip->flags & 2) == 0) && (bForceStrip$1330 == 0)) {
			...
			for (curStripIndex = 0; curStripIndex < stripMeshCountB; curStripIndex = curStripIndex + 3) {
				char* pVifList = reinterpret_cast<char*>(pStrip) + pStrip->vifListOffset;

				pVfiListA = pVifList + iVar20 * curStripIndex * 0x10;

				pTexturePkt = LOAD_POINTER_CAST(edpkt_data*, pMaterial->pCommandBufferTexture) + gVRAMBufferFlush * pMaterial->commandBufferTextureSize * (uint)pMaterial->nbLayers;
				...
				for (uVar25 = 0; uVar25 != mode; uVar25 = uVar25 + 1) {
					...
					if (gGlobalAlhaON != 0x80) {
						if (uVar25 == 0) {
							pCurPkt = ed3DHierachyCheckForGlobalAlphaSetPKT_(pCurPkt, gCurLayer);
						}
						else {
							peVar16 = ed3DG2DMaterialGetLayer(*g_pCurFlareMaterial, (uint)uVar25);
					...
					if (curStripIndex < stripMeshCountB) {
						...
						peVar27 = pCurPkt + 1;
						if (uVar25 == 0) {
							peVar27->cmdA = ((long)(int)pVfiListA & 0xfffffffU) << 0x20 | 0x50000000;
							pCurPkt[1].cmdB = 0;
							uVar30 = pVfiListA + iVar20 * 0x10;
							peVar27 = pCurPkt + 2;
						}
						uVar22 = uVar28 & 0xfffffff;
						uVar28 = uVar28 + 0x50;
						peVar27->cmdA = (ulong)uVar22 << 0x20 | 0x50000000;
					...
					if (curStripIndex + 1 < stripMeshCountB) {
						...
						peVar27 = pCurPkt + 1;
						if (uVar25 == 0) {
							uVar22 = uVar30 & 0xfffffff;
							uVar30 = uVar30 + iVar20 * 0x10;
							peVar27->cmdA = (ulong)uVar22 << 0x20 | 0x50000000;
							pCurPkt[1].cmdB = 0;
							peVar27 = pCurPkt + 2;
						}
						...
						if (uVar25 == (long)(int)(pMaterial->nbLayers - 1)) {
							ed3DSwapVU1Buffer();
						}
					}
					...
					if (curStripIndex + 2 < stripMeshCountB) {
						...
						peVar27 = pCurPkt + 1;
						if (uVar25 == 0) {
							peVar27->cmdA = (ulong)(uVar30 & 0xfffffff) << 0x20 | 0x50000000;
							pCurPkt[1].cmdB = 0;
							peVar27 = pCurPkt + 2;
						}
						peVar27->cmdA = (ulong)(uVar28 & 0xfffffff) << 0x20 | 0x50000000;
						peVar27->cmdB = ((long)iVar21 & 0xffffffffU | 0x4000000) << 0x20;
						pCurPkt = peVar27 + 1;
					}
					else {
			...
			while (uVar25 < mode) {
				...
				pTexturePkt = LOAD_POINTER_CAST(edpkt_data*, pMaterial->pCommandBufferTexture) + gVRAMBufferFlush * (pMaterial->commandBufferTextureSize * curStripIndex) * (uint)pMaterial->nbLayers;
				...
				if (gGlobalAlhaON != 0x80) {
					if (uVar25 == 0) {
						...
						peVar16 = ed3DG2DMaterialGetLayer(*g_pCurFlareMaterial, curStripIndex);
						pCurPkt = ed3DHierachyCheckForGlobalAlphaSetPKT_(pCurPkt, peVar16);
					}
				}
		...
		while (peVar27 = pCurPkt + 2, uVar25 < mode) {
			...
			peVar18 = pMaterial->pCommandBufferTexture;
			curStripIndex = (uint)uVar25;
			iVar29 = piVar5[curStripIndex * iVar6 * 4 + 2];
			local_40 = (int)&pStrip->flags + pStrip->vifListOffset;
...
void ed3DFlushStrip(edNODE* pNode)
{
	...
	if (pNextNode == (edNODE*)0x0) {
		...
		if ((p3dStrip->flags & 4) == 0) {
			...
			piVar14 = LOAD_POINTER_CAST(int*, p3dStrip->pColorBuf);
			...
			for (partialMeshSectionCount = 0; partialMeshSectionCount < (ushort)p3dStrip->meshCount; partialMeshSectionCount = partialMeshSectionCount + 1) {
				...
				pVifRefPktCur[1].cmdA = ED_VIF1_SET_TAG_REF(1, STORE_POINTER(&g_stVertexRGBAHeader));
				pVifRefPktCur[1].cmdB = 0;

				pVifRefPktCur[2].cmdA = ED_VIF1_SET_TAG_REF(uVar23, STORE_POINTER(piVar22));
				pVifRefPktCur[2].asU32[2] = SCE_VIF1_SET_NOP(0);
				pVifRefPktCur[2].asU32[3] = SCE_VIF1_SET_UNPACK(0xc002, uVar23, UNPACK_V4_8_MASKED, 0);

				pVifRefPktCur[3].cmdA = ED_VIF1_SET_TAG_REF(1, STORE_POINTER(&g_stNormalHeader));
				pVifRefPktCur[3].cmdB = 0;

				pVifRefPktCur[4].cmdA = ED_VIF1_SET_TAG_REF(uVar23, STORE_POINTER(piVar21));
			...
			while (pRVar25 < pPktBufferB) {
				...
				LAB_002a5a74:
					...
					*(undefined4*)((ulong)&ppuVar19[2].cmdB + 4) = uVar17;
					...
					ppuVar19[4].cmdA = ED_VIF1_SET_TAG_REF(1, STORE_POINTER(&g_stVertexRGBAHeader));
					ppuVar19[4].cmdB = 0;
					...
					ppuVar19[6].cmdA = ED_VIF1_SET_TAG_REF(1, STORE_POINTER(&g_stNormalHeader));
					ppuVar19[6].cmdB = 0;
...
byte bShowFlare$1221 = 1;
int zPos$1247 = 0x1f4;
uint gFlareTexAddr;
byte bCenterPos$1246 = 0;
bool g_bShowFlare = false;
...
edpkt_data* ed3DFlushSunFlareCreateMarker(float param_1, float param_2, float param_3, float param_4, float param_5, edpkt_data* pPkt)
{
	...
	uVar1 = __fixunssfdi((uint)((param_1 - param_4) * 16.0f));
...
edpkt_data* ed3DFlushSunFlareDoMipmap(float param_1, float param_2, float param_3, edpkt_data* pPkt)
{
	...
	if (g_bShowFlare == false) {
		pPkt->cmdA = (ulong)(uint)gDRAWBUF_BASE_BIS | 0xffffff00080000;
		pPkt->cmdB = 0x4d;
	}
	else {
		pPkt->cmdA = (ulong)(uint)gDRAWBUF_BASE_BIS | 0x80000;
		pPkt->cmdB = 0x4d;
	}

	pPkt[1].cmdA = (ulong)gZBUF_BASE | 0x31000000;
	pPkt[1].cmdB = 0x4f;

	pPkt[2].cmdA = 0x60;
	pPkt[2].cmdB = 0x15;

	uVar3 = (ulong)gDRAWBUF_TXBASE;
	pPkt[3].cmdA = uVar3 | 0xe64020000;
	pPkt[3].cmdB = 7;
...
edpkt_data* ed3DFlushSunFlareGetIntensity(edpkt_data* pPkt, ed_g2d_bitmap* pBitmap)
{
	...
	if (psm != 4) {
		if (psm == 8) {
			peVar3->cmdA = (ulong)(int)(gFlarePalAddr >> 5) | 0xffffff00010000;
			pPkt[1].cmdB = 0x4d;
			peVar3 = pPkt + 2;
		}
		else {
			if ((psm == 0x18) || (psm == 0x20)) {
				peVar3->cmdA = (ulong)(int)(gFlareTexAddr >> 5) | (ulong)(int)(uint)(pBitmap->width >> 6) << 0x10 | 0xffffff00000000;
				pPkt[1].cmdB = 0x4d;
				peVar3 = pPkt + 2;
			}
		}
...
edpkt_data* ed3DFlushSunFlare2Screen(float param_1, float param_2, float param_3, edpkt_data* pPkt, ed_g2d_bitmap* pBitmap, int param_6, uint param_7)
{
	...
	pPkt[4].cmdA = (ulong)(uint)param_1 << 0x20 | 0x405058;
	pPkt[4].cmdB = 1;
	...
	if (psm != 4) {
		if (psm == 8) {
			psm = pBitmap->width;
			uVar4 = (ulong)gFlareTexAddr;
			iVar2 = GetPowerVal((uint)psm);
			iVar3 = GetPowerVal((uint)pBitmap->height);
			pNxtPkt->cmdA = (ulong)gFlarePalAddr << 0x25 |
				uVar4 | (ulong)(int)(uint)(psm >> 6) << 0xe | 0x1300000 | ((ulong)iVar2 & 0xffffffffU) << 0x1a |
				((ulong)iVar3 & 0xffffffffU) << 0x1e | 0x400000000;
			pPkt[5].cmdB = 6;
			...
			iVar2 = GetPowerVal((uint)pBitmap->width);
			psm = pBitmap->width;
			uVar4 = (ulong)gFlareTexAddr;
			iVar3 = GetPowerVal((uint)pBitmap->height);
			pNxtPkt->cmdA = uVar4 | (ulong)(int)(uint)(psm >> 6) << 0xe | ((ulong)iVar2 & 0xffffffffU) << 0x1a |
				((ulong)iVar3 & 0xffffffffU) << 0x1e | 0x400000000;
			pPkt[5].cmdB = 6;
...
LAB_002b03a4:
	...
	uVar4 = __fixunssfdi((uint)((param_2 - fVar8) * 16.0f));
	if ((int)param_7 < 0) {
		fVar8 = (float)(param_7 >> 1 | param_7 & 1);
		fVar8 = fVar8 + fVar8;
	}
	...
	uVar5 = __fixunssfdi((uint)((param_3 - fVar8) * 16.0f));
	...
	uVar5 = __fixunssfdi((uint)((param_2 + fVar8) * 16.0f));
	if ((int)param_7 < 0) {
		fVar8 = (float)(param_7 >> 1 | param_7 & 1);
		fVar8 = fVar8 + fVar8;
	}
	...
	uVar6 = __fixunssfdi((uint)((param_3 + fVar8) * 16.0f));
...
edpkt_data* ed3DFlushSpriteFlareFX(float param_1, edpkt_data* pPkt, edF32VECTOR4* pVtx, byte param_4)
{
	edSurface* pZBuffer;
	edSurface* pDrawSurface;
	ed_g2d_bitmap* pBitmap;
	edpkt_data* peVar4;
	int iVar5;
	float fVar6;
	...
	if ((bShowFlare$1221 != 0) && (gCurViewportUsed != (ed_viewport*)0x0)) {
		pDrawSurface = edVideoGetDrawSurface();
		pZBuffer = gCurViewportUsed->pZBuffer;

		if ((pDrawSurface != (edSurface*)0x0) &&
			((*g_pCurFlareMtx != (edF32MATRIX4*)0x0 && (*g_pCurFlareMaterial != (ed_g2d_material*)0x0)))) {
			pBitmap = ed3DGetG2DBitmap(*g_pCurFlareMaterial, 0);
			gOFFX = ((int)(0x1000 - (uint)pDrawSurface->pSurfaceDesc->screenWidth) >> 1) << 4;
			gOFFY = ((int)(0x1000 - (uint)pDrawSurface->pSurfaceDesc->screenHeight) >> 1) << 4;
			gDRAWBUF_BASE = pDrawSurface->frameBasePtr;
			gDRAWBUF_TXBASE = gDRAWBUF_BASE << 5;
			gZBUF_BASE = pZBuffer->frameBasePtr;
			gZBUF_BASE_BIS_TEX = 0x800;
			gZBUF_BASE_BIS = 0x40;
			gZBUF_TEXBASE = gZBUF_BASE << 5;
			gSCRN_W = (uint)pDrawSurface->pSurfaceDesc->screenWidth;
			gSCRN_H = (uint)pDrawSurface->pSurfaceDesc->screenHeight;
			if (gVRAMBufferFlush == 0) {
				gFlareTexAddr = gVRAMBuffers[0];
			}
			else {
				gFlareTexAddr = gVRAMBuffers[1];
			}
			...
			local_10.z = local_10.z * local_10.w;
			...
			if (0.0f <= local_10.z) {
				if (bCenterPos$1246 == 0) {
					if (param_4 == 0) {
						...
						edF32Matrix4MulF32Vector4Hard(&local_20, WorldToCamera_Matrix, &local_20);
						local_20.x = local_20.x - param_1;
						edF32Matrix4MulF32Vector4Hard(&local_20, CameraToScreen_Matrix, &local_20);

						iVar5 = (int)(local_20.x * local_10.w - local_10.x);
						if (iVar5 < 0) {
							iVar5 = -iVar5;
						}
				...
				fVar9 = 1.0f / local_10.z;
				if ((((local_10.x <= 2700.0f) && (1300.0f <= local_10.x)) && (local_10.y <= 2700.0f)) && (1300.0f <= local_10.y)) {
					fVar8 = local_10.x;
					if (local_10.x < 1800.0f) {
						fVar8 = 1800.0f;
					}
					...
					if (param_1 == 512.0f) {
						...
						pPkt->cmdB = 0;
						pPkt->asU32[2] = 0;
						pPkt->asU32[3] = 0x50000002;

						pPkt[1].cmdA = 0x1000000000008001;
						pPkt[1].cmdB = 0xe;

						pPkt[2].cmdA = 0x700000007000;
						pPkt[2].cmdB = 0x18;
					...
					if (param_1 < 2.147484e+09f) {
						uVar10 = (uint)param_1;
					}
					else {
						uVar10 = (int)(param_1 - 2.147484e+09f) | 0x80000000;
					}
...
edpkt_data* ed3DFlushSpriteScaleFlare(ed_3d_sprite* pSprite, edpkt_data* pPkt)
{
	...
	pVtx = LOAD_POINTER_CAST(edVertex*, pSprite->pVertexBuf);
	puVar3 = LOAD_POINTER_CAST(uint*, pSprite->pWHBuf);
...
edpkt_data* ed3DRefreshFANGifTag(edpkt_data* pPkt, uint flags, ed_g2d_material* pMaterial)
{
	...
	if ((flags & 2) != 0) {
		...
		if (pOldFANGifTag != pGifTag) {
			...
			pPkt[1].asU32[1] = STORE_POINTER(pGifTag);
			pPkt[1].asU32[2] = SCE_VIF1_SET_NOP(0);
...
void ed3DFlushStripList(edLIST* pList, ed_g2d_material* pMaterial)
{
	...
	ED3D_LOG(LogLevel::Verbose, "ed3DFlushStripList pList: 0x{:x} pMaterial: 0x{:x}", (uintptr_t)pList, (uintptr_t)pMaterial);
	...
	if ((pMaterial == &gMaterial_Render_Zbuffer_Only_Cluster) || (pMaterial == &gMaterial_Render_Zbuffer_Only)) {
		...
		if (gShadowFlushMode == 0) {
			if (pMaterial->nbLayers < 2) {
				...
				while (pNode != pList) {
					...
					LAB_002a7d54:
						...
						if ((listNodeType == LIST_TYPE_STRIP) || (listNodeType == LIST_TYPE_PKT_SPRITE)) {
							ed3DFlushOptionState(pNode);
							goto LAB_002a7d54;
						}

						if (listNodeType == LIST_TYPE_STRIP) {
							IMPLEMENTATION_GUARD(
								if ((*pNode->pData & 0x10000) == 0) {
									ed3DFlushStripForZbufferOnly((edNODE*)pNode, (byte*)pMaterial);
								}
								else {
									bVar3 = true;
			...
			while (pNode != pList) {
				...
				if ((listNodeType == LIST_TYPE_PKT_STRIP) || (listNodeType == LIST_TYPE_PKT_SPRITE)) {
					ed3DFlushOptionState(pNode);
				LAB_002a7c6c:
					pNode = pNode->pPrev;
				}
					...
					if (listNodeType == LIST_TYPE_STRIP) {
						ed3DFlushStripShadowRender((edNODE*)pNode, pMaterial);
						goto LAB_002a7c6c;
					}
		...
		if (pMaterial->nbLayers < 2) {
			for (; pNode != pList; pNode = pNode->pPrev) {
				listNodeType = pNode->header.typeField.type;
				...
				if ((listNodeType == LIST_TYPE_PKT_STRIP) || (listNodeType == LIST_TYPE_PKT_SPRITE)) {
					...
					if (listNodeType == LIST_TYPE_SPRITE) {
						ed3DFlushSprite(pNode, pMaterial);
					}
					else {
						if (listNodeType == LIST_TYPE_STRIP) {
							ed3DFlushStrip(pNode);
			...
			for (; pNode != pList; pNode = pNode->pPrev) {
				...
				if (listNodeType == LIST_TYPE_SPRITE) {
					...
					if (listNodeType == LIST_TYPE_STRIP) {
						ed3DFlushStripMultiTexture(pNode, pMaterial);
					}
				}
...
ed_g2d_layer* ed3DGetG2DLayer(ed_g2d_material* pMAT, int index)
{
	...
	if (pMAT != (ed_g2d_material*)0x0) {
		if (pMAT->nbLayers == 0) {
			...
			pTVar1 = (ed_g2d_layer*)((ed_Chunck*)LOAD_POINTER(pMAT->aLayers[index]) + 1);
		}
	}
	return pTVar1;
}
	...
	if ((pMaterial->flags & 1) == 0) {
		...
		if (pNode != (edNODE*)0x0) {
			if (gShadowFlushMode == 0) {
				...
				for (; pNode != &pMaterial->list; pNode = pNode->pPrev) {
					ed3DFlushMatrixForShadow((ed_dma_matrix*)pNode->pData);
					ed3DFlushMatrix((ed_dma_matrix*)pNode->pData, pMaterial->pMaterial);
				}
			...
			pMaterial->list.pPrev = &pMaterial->list;
			pMaterial->list.pNext = &pMaterial->list;
		}
...
{
	ed_hash_code* pBitmapHash = LOAD_POINTER_CAST(ed_hash_code*, pTexture->hashCode.pData);
	ed_Chunck* pT2D = LOAD_POINTER_CAST(ed_Chunck*, pBitmapHash->pData);
...
edpkt_data* ed3DFlushMaterialAnimST(edpkt_data* pPkt)
{
	...
	if ((layerFlags & 0x10) == 0) {
		if ((layerFlags & 2) != 0) {
			pTEX = LOAD_POINTER_CAST(ed_Chunck*, gCurLayer->pTex);

			ed_g2d_texture* pTexture = reinterpret_cast<ed_g2d_texture*>(pTEX + 1);

			ed3DG2DGetBitmapFromTexture(pTexture);

			ed_Chunck* p2dAnimChunk = LOAD_POINTER_CAST(ed_Chunck*, pTexture->pAnimChunck);
			...
			pPkt = pPkt + 2;
			gCurLayer->flags_0x4 = gCurLayer->flags_0x4 | LAYER_FLAG_ANIM_ST_UPDATED;

			Renderer::PushAnimST(gVU1_AnimST_NormalExtruder_Scratch->raw);
		}
	}
	else {
		pTEX = (ed_Chunck*)LOAD_POINTER(gCurLayer->pTex);
		if (pTEX != (ed_Chunck*)0xfffffff0) {
			ed_g2d_texture* pTexture = reinterpret_cast<ed_g2d_texture*>(pTEX + 1);
			peVar5 = LOAD_POINTER_CAST(edF32VECTOR4*, pTexture->pAnimSpeedNormalExtruder);

			if (((layerFlags & LAYER_FLAG_ANIM_ST_UPDATED) == 0) && (gStepTime != 0)) {
				ED3D_LOG(LogLevel::Verbose, "ed3DLinkStripToViewport tex: {} Updating ANIM ST", pTexture->hashCode.hash.ToString());
	...
	if (gFushListCounter != 0xe) {
		if ((gbFirstTex == 0) && (pMaterial->pBitmap != (ed_g2d_bitmap*)0x0)) {
			if (pMaterial->pMaterial->nbLayers < 2) {
				...
				edPSX2Header* pHeader = (edPSX2Header*)LOAD_POINTER(pMaterial->pBitmap->pPSX2);
				edpkt_data* pPkt = (edpkt_data*)(LOAD_POINTER((pHeader + gVRAMBufferFlush)->pPkt));
...
#endif
				peVar4[1].cmdA = ED_VIF1_SET_TAG_REF(peVar1->commandBufferTextureSize, STORE_POINTER(pFlippedPkt));
...
bool ed3DCheckSpherePacket(ed_3d_strip* param_1)
{
	...
	pfVar8 = (ed_Bound_Sphere_packet*)LOAD_POINTER(param_1->pBoundSpherePkt);
	if (pfVar8 == (ed_Bound_Sphere_packet*)0x0) {
		...
		uVar10 = 0;
		param_1->pDMA_Matrix.pDMA_Matrix = (int)STORE_POINTER(ed3D_SubAllocator_004491b8.current);
		pRVar11 = (ed_dma_matrix*)LOAD_POINTER(param_1->pDMA_Matrix.pDMA_Matrix);
		uVar9 = 0;
...
void ed3DLinkStripManageLinkToDMALink(edLIST* pList, ed_dma_material* pMaterial, short type)
{
	...
	pList->nodeCount = pList->nodeCount + 1;
	peVar2->header.typeField.type = type;
	peVar2->pPrev = peVar4;
	peVar3->pPrev = peVar2;
...
LAB_00297680:
	if ((pStripMaterial != (ed_g2d_material*)0x0) && (pStripMaterial->nbLayers != 0)) {
		...
		pLAY = LOAD_POINTER_CAST(ed_Chunck*, pStripMaterial->aLayers[0]);
		ed_g2d_layer* pLayer = reinterpret_cast<ed_g2d_layer*>(pLAY + 1);
		...
		if (pLayer->bHasTexture != 0) {
			...
			if (pTexture->bHasPalette == 0) {
				...
				if (iVar4 != 0) {
					ed_hash_code* pPaletteHashCode = (ed_hash_code*)LOAD_POINTER(iVar4);
...
LAB_00297870:
	if ((((layerFlagsA & 0xfc) == 0) || ((layerFlagsA & 0x80000000) != 0)) || ((layerFlagsA & 0x4000) != 0)) {
		pExistingDMA_Material = (ed_dma_material*)LOAD_POINTER(pStripMaterial->pDMA_Material);
		if (pStripMaterial->pDMA_Material == (int)0x0) {
			pDMA_Material = ed3DListCreateDmaMaterialNode(0.0f, pStripMaterial);
			pDMA_Material->flags = pDMA_Material->flags | 1;
			pStripMaterial->pDMA_Material = STORE_POINTER(pDMA_Material);
			...
			ed3DLinkMaterialToPrimList(pDMA_Material, pStrip->primListIndex);
		...
		pExistingDMA_Material = ed3DListCreateDmaMaterialNode(gBoundSphereCenter->z, pStripMaterial);
		pExistingDMA_Material->flags = pExistingDMA_Material->flags | 1;
		pExistingDMA_Material->pBitmap = pBitmap;

		ed3DLinkStripManageLinkToDMALink(gPrim_List[3] + gCurRenderList, pExistingDMA_Material, LIST_TYPE_MATERIAL);

		ED3D_LOG(LogLevel::Verbose, "ed3DLinkClusterStripToViewport Linked Cluster Strip: ({}, {}). New node count: {}", pStrip->primListIndex, gCurRenderList,
			gPrim_List[pStrip->primListIndex][gCurRenderList].nodeCount);
	}

	ed3DLinkStripToList(&pExistingDMA_Material->list, pStrip);
...
ed3DLod* ed3DClusterChooseGoodLOD(ed_g3d_hierarchy* pHier, edF32VECTOR4* pSphere)
{
	...
	if (pHier->lodCount == 0) {
		...
		if ((pHier->pHierarchySetup == 0x0) || (pLodBiases = (LOAD_POINTER_CAST(ed_3d_hierarchy_setup*, pHier->pHierarchySetup))->pLodBiases, pLodBiases == (float*)0x0)) {
			pLod = pHier->aLods;
			lodIndex = 0;
...
LAB_002b181c:
	...
	*gBoundSphereCenter = (*gBoundSphereCenter) * (*gRender_info_SPR->pMeshTransformMatrix);
	...
	ED3D_LOG(LogLevel::VeryVerbose, "ed3DRenderClusterObject bounding sphere test result: {}", (int)EVar3);
	if (EVar3 != VM_4) {
...
void ed3DRenderClusterHierarchy(ed_g3d_hierarchy* pHier)
{
	...
	if (((pHier->flags_0x9e & 0x40) == 0) && (pHier->lodCount != 0)) {
		gRender_info_SPR->pHierarchySetup = (ed_3d_hierarchy_setup*)LOAD_POINTER(pHier->pHierarchySetup);
		...
		if (peVar3 != (edF32VECTOR4*)0x0) {
			...
			if (pLod != (ed3DLod*)0x0) {
				pcVar2 = (char*)LOAD_POINTER(pHier->pTextureInfo);
				...
				pLodHash = (ed_hash_code*)LOAD_POINTER(pLod->pObj);
				...
				if (pLodHash != (ed_hash_code*)0x0) {
					...
					if (sVar1 == 3) {
						...
						if ((sVar1 != 2) && (sVar1 != 1)) {
							if (sVar1 != 0) {
								peVar5 = LOAD_POINTER_CAST(ed_hash_code*, pHier->pLinkTransformData);
								goto LAB_002b1758;
							}
...
void ed3DLinkStripManageLinkToDMA(ed_3d_strip* pStrip, uint flagsA, uint flagsB, edF32VECTOR4* pBoundingSphere, ed_g2d_material* pLinkMaterial, ed_g2d_bitmap* pBitmap)
{
	...
	ED3D_LOG(LogLevel::Verbose, "ed3DLinkStripManageLinkToDMA Linking Strip: 0x{:x} To Material: 0x{:x}", (uintptr_t)pStrip, (uintptr_t)pLinkMaterial);
	...
	if ((((flagsB & 0x200) != 0) && ((flagsA & 0x80000000) != 0)) ||
		...
		pCurrentMaterial = ed3DListCreateDmaMaterialNode(pBoundingSphere->z, pLinkMaterial);
		ed_dma_matrix* pNewMatrixNode = ed3DListCreateDmaMatrixNode(gRender_info_SPR, gRender_info_SPR->pMeshTransformData);
		...
		ed3DListAddNodeSorted(&pCurrentMaterial->list, pNewMatrixNode, LIST_TYPE_MATRIX);

		if (gRender_info_SPR->pPkt != (edpkt_data*)0x0) {
			ed3DListAddNodeSorted(pNewMatrixNode, gRender_info_SPR->pPkt, LIST_TYPE_PKT_STRIP);
		}

		ed3DLinkStripToList(pNewMatrixNode, pStrip);

		return;
	}

	/* Boomy path */
	pExistingDmaMaterial = LOAD_POINTER_CAST(ed_dma_material*, pLinkMaterial->pDMA_Material);
	if (pExistingDmaMaterial == (ed_dma_material*)0x0) {
		pCurrentMaterial = ed3DListCreateDmaMaterialNode(0.0f, pLinkMaterial);
		pprevious_dma_matrix = ed3DListCreateDmaMatrixNode(gRender_info_SPR, gRender_info_SPR->pMeshTransformData);
		pLinkMaterial->pDMA_Material = STORE_POINTER(pCurrentMaterial);
		pCurrentMaterial->pBitmap = pBitmap;
		...
		ed3DListAddNodeSorted(&pCurrentMaterial->list, pprevious_dma_matrix, LIST_TYPE_MATRIX);
	}
	else {

		if ((pprevious_dma_material != pExistingDmaMaterial) && ((pExistingDmaMaterial->list).nodeCount != 0)) {
			pprevious_dma_matrix = (ed_dma_matrix*)((pExistingDmaMaterial->list).pPrev)->pData;
		}

		pCurrentMaterial = pExistingDmaMaterial;
...
LAB_002992a8:
	...
	ed3DLinkStripToList(pprevious_dma_matrix, pStrip);

	if (gRender_info_SPR->pPkt != 0x0) {
		ed3DListAddNodeSorted(pprevious_dma_matrix, gRender_info_SPR->pPkt, LIST_TYPE_PKT_STRIP);
	}
...
void _ed3DLinkStripToViewport(ed_3d_strip* pStrip, ed_hash_code* pMBNK)
{
	...
	ED3D_LOG(LogLevel::Verbose, "_ed3DLinkStripToViewport strip: 0x{:x}", (uintptr_t)pStrip);

	pMaterial = (ed_g2d_material*)0x0;
	if (pStrip->materialIndex != -1) {
		pMaterial = ed3DG2DGetG2DMaterialFromIndex(pMBNK, pStrip->materialIndex);
		if (((gRender_info_SPR->pMeshTransformData != (ed_3d_hierarchy*)0x0) &&
				...
				if (uVar8 != 1) {
					...
					if ((pStrip->flags & 0x1000) == 0) {
						...
						if (EVar6 == VM_1) {
							...
							if ((pMaterial == (ed_g2d_material*)0x0) ||
								((pMaterial != (ed_g2d_material*)0x0 && (pMaterial->nbLayers < 2)))) {
								lVar10 = ed3DCheckSpherePacket(pStrip);
							joined_r0x00297e70:
								bVar4 = false;
...
LAB_00297eec:
	...
	if ((pMaterial != (ed_g2d_material*)0x0) && (pMaterial->nbLayers != 0)) {
		pLAY = LOAD_POINTER_CAST(ed_Chunck*, pMaterial->aLayers[0]);

		ed_g2d_layer* pLayer = reinterpret_cast<ed_g2d_layer*>(pLAY + 1);
		layerFlags0 = pLayer->flags_0x0;
		layerFlags1 = pLayer->flags_0x4;
		pBitMap = 0;
		...
		if (pLayer->bHasTexture != 0) {
			...
			if (pTexture->bHasPalette == 0) {
				ed_hash_code* pTextureHashCode = (ed_hash_code*)LOAD_POINTER(pTexture->hashCode.pData);
				if (pTextureHashCode != (ed_hash_code*)0x0) {
					ed_Chunck* pT2D = LOAD_POINTER_CAST(ed_Chunck*, pTextureHashCode->pData);
					pBitMap = reinterpret_cast<ed_g2d_bitmap*>(pT2D + 1);
				}
				...
				pPaletteHashCode = (ed_hash_code*)LOAD_POINTER(pPaletteHashCode[pLayer->paletteId].pData);
			...
			if ((pStrip->flags & 4) != 0) {
				ed3DManageAnim(ed3DG3DAnimGet(pStrip));
			}

			goto LAB_002980e4;
		}

		if ((layerFlags1 & 0x800) != 0) goto LAB_002980e4;
	}
...
LAB_002980e4:
	ed3DLinkStripManageLinkToDMA(pStrip, layerFlags0, layerFlags1, &eStack16, pMaterial, pBitMap);
...
void _ed3DLinkSpriteToViewport(ed_3d_sprite* pSprite, ed_hash_code* pMBNK)
{
	...
	int iVar2;
	ed_dma_material* pExistingDmaMaterial;
	ed_3d_hierarchy* peVar4;
	ed_g2d_material** ppeVar5;
	ed_dma_material* pDMA_Material;
	ed_g2d_material* pMaterial;
	uint layerFlagsA;
	edLIST* peVar9;
	ed_dma_matrix* pNewMatrixNode;
	undefined2* puVar12;
	edNODE* peVar13;
	int* piVar14;
	uint layerFlagsB;
	...
	if (((*gShadowRenderMask == 0) || ((*gShadowRenderMask & (uint)(ushort)(pSprite->bUseShadowMatrix_0x30 | pSprite->field_0x32)) != 0)) &&
		((pSprite->materialIndex == 0xffff || ((pMaterial = ed3DG2DGetG2DMaterialFromIndex(pMBNK, pSprite->materialIndex), pMaterial == (ed_g2d_material*)0x0 || ((pMaterial->flags & 1) == 0)))))) {
		if (gRender_info_SPR->boundingSphereTestResult == 8) {
				...
				*gBoundSphereCenter = (*gBoundSphereCenter) * (*gRender_info_SPR->pMeshTransformMatrix);
				...
				if (layerFlagsA != 1) {
					...
					if (layerFlagsA == 1) {
						...
						layerFlagsA = pSprite->flags_0x0;
						if ((layerFlagsA & 0x1000) == 0) {
							pSprite->flags_0x0 = layerFlagsA | 3;
						}
						else {
							pSprite->flags_0x0 = layerFlagsA | 1;
						}
					}
				}
			}
		...
		if ((pMaterial == (ed_g2d_material*)0x0) || (pMaterial->nbLayers == 0)) {
			...
			if (((pLayer->bHasTexture == 0) || ((*gShadowRenderMask != 0 && ((layerFlagsA & 0x100) == 0)))) && ((layerFlagsB & 0x800) == 0)) {
				layerFlagsA = 0;
				layerFlagsB = 0;
				pMaterial = gDefault_Material_Cluster_Current;
			}
		...
		pExistingDmaMaterial = LOAD_POINTER_CAST(ed_dma_material*, pMaterial->pDMA_Material);
		if (pExistingDmaMaterial == (ed_dma_material*)0x0) {
			pDMA_Material = ed3DListCreateDmaMaterialNode(0.0f, pMaterial);
			pNewMatrixNode = ed3DListCreateDmaMatrixNode(gRender_info_SPR, 0);
			pMaterial->pDMA_Material = STORE_POINTER(pDMA_Material);
			...
			if ((layerFlagsB & 0x800) == 0) {
				if ((pSprite->pRenderFrame30 & 0x200) == 0) {
					if ((layerFlagsB & 0x40) == 0) {
						if (((layerFlagsA & 0x80000000) == 0) && ((pSprite->flags_0x0 & 0x280) == 0)) {
							if ((layerFlagsA & 0x4000) == 0) {
								if ((layerFlagsA & 0x100) == 0) {
									ed3DLinkMaterialToPrimList(pDMA_Material, gCurRenderList);
								}
								else {
									ed3DLinkMaterialToPrimList(pDMA_Material, PRIM_LIST_2);
								}
							}
							else {
								ed3DLinkMaterialToPrimList(pDMA_Material, PRIM_LIST_4);
							}
						}
						else {
							ed3DLinkMaterialToPrimList(pDMA_Material, PRIM_LIST_6);
						}
					}
					else {
						if ((layerFlagsA & 0x80000000) == 0) {
							if ((layerFlagsA & 0x4000) == 0) {
								if ((layerFlagsA & 0xfc) == 0) {
									if ((layerFlagsA & 0x100) == 0) {
										ed3DLinkMaterialToPrimList(pDMA_Material, PRIM_LIST_1);
									}
									else {
										ed3DLinkMaterialToPrimList(pDMA_Material, PRIM_LIST_2);
									}
									...
									ed3DLinkMaterialToPrimList(pDMA_Material, PRIM_LIST_3);
								}
							}
							else {
								ed3DLinkMaterialToPrimList(pDMA_Material, PRIM_LIST_4);
					...
					ed3DLinkMaterialToPrimList(pDMA_Material, PRIM_LIST_A);
				}
			}
			else {
				ed3DLinkMaterialToPrimList(pDMA_Material, PRIM_LIST_D);
			}

			ed3DListAddNodeSorted(&pDMA_Material->list, pNewMatrixNode, LIST_TYPE_MATRIX);

			ed3DLinkSpriteToList(pNewMatrixNode, pSprite);

			pprevious_dma_matrix = pNewMatrixNode;
			pprevious_dma_material = pDMA_Material;

			if (gRender_info_SPR->pPkt != (edpkt_data*)0x0) {
				ed3DListAddNodeSorted(pNewMatrixNode, gRender_info_SPR->pPkt, LIST_TYPE_PKT_SPRITE);
			}
		}
		else {
			if ((pprevious_dma_matrix->pObjToWorld == gRender_info_SPR->pSharedMeshTransform) && (pprevious_dma_material == pExistingDmaMaterial)) {
				ed3DLinkSpriteToList(pprevious_dma_matrix, pSprite);
				pprevious_dma_material = pExistingDmaMaterial;

				if (gRender_info_SPR->pPkt != (edpkt_data*)0x0) {
					ed3DListAddNodeSorted(pprevious_dma_matrix, gRender_info_SPR->pPkt, LIST_TYPE_PKT_SPRITE);
				}
			}
			else {
				pprevious_dma_matrix = ed3DListCreateDmaMatrixNode(gRender_info_SPR, 0);
				ed3DListAddNodeSorted(&pExistingDmaMaterial->list, pprevious_dma_matrix, LIST_TYPE_MATRIX);
				ed3DLinkSpriteToList(pprevious_dma_matrix, pSprite);
		...
		*flagsB = pLayer->flags_0x4;
		...
		if ((pLayer->bHasTexture != 0) && ((*gShadowRenderMask == 0 || ((((*flagsA & 0x100) != 0 && ((*ppMaterial)->nbLayers < 2)) || ((pStrip->flags & 0x10000) != 0)))))) {
			...
			if (pTexture->bHasPalette == 0) {
				...
				ed_hash_code* pPaletteHashCode = (ed_hash_code*)(pTexture + 1);
				pPaletteHashCode = LOAD_POINTER_CAST(ed_hash_code*, pPaletteHashCode[pLayer->paletteId].pData);
...
LAB_002a335c:
	...
	for (batchIndex = 0; batchIndex < nbBatches; batchIndex = batchIndex + 1) {
		...
		pPkt[1].asU32[0] = ED_VIF1_SET_TAG_REF(1, 0);
		pPkt[1].asU32[1] = STORE_POINTER(pGifTag + nbVertices);
	...
	pSprite->pNext = STORE_POINTER(pPkt);
...
void ed3DRenderSprite(ed_hash_code* pLOD, ed_hash_code* pMBNK)
{
	...
	ed_3d_sprite* pRenderInput = LOAD_POINTER_CAST(ed_3d_sprite*, pMeshOBJ->p3DData);
	if (pRenderInput == (ed_3d_sprite*)0x0) {
		return;
	}

	int stripCount = pMeshOBJ->stripCount;

	edF32VECTOR4* pSphere;
	int defaultBoundSphereTestResult;
	ushort flags;

	if ((gRender_info_SPR->flags & 0x10) != 0) {
		pSphere = gRender_info_SPR->pHierarchySetup->pBoundingSphere;
		defaultBoundSphereTestResult = 8;
		if (pSphere != (edF32VECTOR4*)0x0) {
			flags = pRenderInput->pRenderFrame30;
			goto LAB_002b0ee8;
		}
	}
	pSphere = &pMeshOBJ->boundingSphere;
	defaultBoundSphereTestResult = 2;
	flags = pRenderInput->pRenderFrame30;
LAB_002b0ee8:
	if ((flags & 0x200) != 0) {
		float fVar8;
		...
		if (INT_00448a54 < 0) {
			fVar8 = (float)(INT_00448a54 >> 1 | INT_00448a54 & 1);
			fVar8 = fVar8 + fVar8;
		}
		else {
	...
	*gBoundSphereCenter = (*gBoundSphereCenter) * (*gRender_info_SPR->pMeshTransformMatrix);
	...
	uint testBoundSphereResult = ed3DTestBoundingSphereObject(pSphere);
	if (testBoundSphereResult != 4) {
		if (testBoundSphereResult == 1) {
			gRender_info_SPR->boundingSphereTestResult = 1;
		}
		else {
			testBoundSphereResult = ed3DTestBoundingSphereObjectNoZFar(pSphere);
			if (testBoundSphereResult != 1) {
				gRender_info_SPR->boundingSphereTestResult = defaultBoundSphereTestResult;
			}
		}

		bool bVar1;
		while (bVar1 = stripCount != 0, stripCount = stripCount + -1, bVar1) {
			_ed3DLinkSpriteToViewport(pRenderInput, pMBNK);
			pRenderInput = LOAD_POINTER_CAST(ed_3d_sprite*, pRenderInput->pNext);
		}
	}
	return;
}
...
void ed3DRenderObject(ed_hash_code* pLOD, ed_hash_code* pMBNK)
{
	...
	pStrip = LOAD_POINTER_CAST(ed_3d_strip*, pMeshOBJ->p3DData);
...
void ed3DRenderSonHierarchy(ed_3d_hierarchy* pHierarchy)
{
	...
	if (((pHierarchy->flags_0x9e & 0x41) == 0) && (pHierarchy->lodCount != 0)) {
		...
		if (pLod != (ed3DLod*)0x0) {
			...
			pObjHash = LOAD_POINTER_CAST(ed_hash_code*, pLod->pObj);
			...
			if (sVar3 == 3) {
				...
				if (((sVar3 != 2) && (sVar3 != 1)) && (sVar3 == 0)) {
					if ((pHierarchy->flags_0x9e & 0x100) == 0) {
						...
						ed3DRenderObject((ed_hash_code*)LOAD_POINTER(pLod->pObj), (ed_hash_code*)(pHierarchy->pTextureInfo + 1));
					}
					else {
...
void ed3DSceneRenderOne(ed_3D_Scene* pShadowScene, ed_3D_Scene* pScene)
{
	...
	if ((pShadowScene->bShadowScene == 1) && ((pScene->flags & 0x10) != 0)) {
		...
		if (ged3DConfig.bEnableProfile != 0) {
			edProfileBegin(gIDProfileFlush);
		}
		...
		if ((uVar6 & 0xff) != 0) {
			...
			if ((pShadowScene->bShadowScene != 1) && (ed3DSceneSortClusters(pShadowScene), (pShadowScene->flags & 8) == 0)) {
				for (pClusterNode = (pScene->meshClusterList).pPrev;
					pClusterNode != &pScene->meshClusterList; pClusterNode = pClusterNode->pPrev) {
	...
	if (bcalcFrame == false) {
		...
		for (curSceneIndex = 0; curSceneIndex < (uint)ged3DConfig.sceneCount; curSceneIndex = curSceneIndex + 1) {
			...
			if ((((sceneFlags & 1) != 0) && ((sceneFlags & 4) == 0)) &&
				...
				for (pCVar12 = (pScene->meshClusterShadowList).pPrev;
					pCVar12 != &pScene->meshClusterShadowList; pCVar12 = pCVar12->pPrev) {
	...
	do {
		if (pBuffer->nbLayers == 0) {
			...
			for (uVar11 = 0; uVar11 < pBuffer->nbLayers; uVar11 = uVar11 + 1) {
				...
				pLAY = LOAD_POINTER_CAST(ed_Chunck*, pHash[uVar11].pData);
				pLayer = reinterpret_cast<ed_g2d_layer*>(pLAY + 1);
				...
				if (pTexture == (ed_g2d_texture*)0x0) {
					...
					pTVar1 = (ed_hash_code*)LOAD_POINTER(pTexture->hashCode.pData);
					if (pTVar1 != (ed_hash_code*)0x0) {
						ed_Chunck* pPA32 = LOAD_POINTER_CAST(ed_Chunck*, pTVar1->pData);
...
void ed3DInit(void)
{
	...
	*(undefined8*)&pTVar1->pT2DA = 0x7ffe000000000;
	...
	if (ged3DConfig.bEnableProfile == 1) {
		gIDProfileRender = edProfileNew(1, 0, 0, 0x80, "3DRend");
		gIDProfileFlush = edProfileNew(1, 0x80, 0, 0, "3DFlush");
	}
...
#define HASH_CODE_INFO 0x4f464e49
...
#if 1
	if (pREAA->hash == HASH_CODE_INFO) {
		char* pText = reinterpret_cast<char*>(pREAA + 1);
		ED3D_LOG(LogLevel::Info, "ed3DPreparePointer INFO chunk - size: 0x{:x} - text: {}", pREAA->size, pText);
	}
#endif
		...
		if (chunkHeader == HASH_CODE_REAL) {
			...
			for (uVar4 = 0; uVar4 < (uVar3 & 0xffff); uVar4 = uVar4 + 1 & 0xffff) {
				...
				if (*nextSubSection != 0) {
					...
					*(int*)(actualFileStart + *nextSubSection) = STORE_POINTER(actualFileStart + offset);
 				}
			...
			if (chunkHeader == HASH_CODE_RSTR) {
				...
				for (uVar4 = 0; uVar4 < (uVar3 & 0xffff); uVar4 = uVar4 + 1 & 0xffff) {
					if (*nextSubSection != 0) {
						/* Store a pointer to the next sub section */
						*(int*)(pcVar5 + *nextSubSection) = (int)STORE_POINTER(actualFileStart + (int)*(char**)(pcVar5 + *nextSubSection));
					}
				...
				if (chunkHeader == HASH_CODE_ROBJ) {
					...
					for (uVar4 = 0; uVar4 < (uVar3 & 0xffff); uVar4 = uVar4 + 1 & 0xffff) {
						if (*nextSubSection != 0) {
							/* Store a pointer to the next sub section */
							*(int*)(actualFileStart + *nextSubSection) = (int)STORE_POINTER(pcVar5 + (int)*(char**)(actualFileStart + *nextSubSection));
						}
					...
					if (chunkHeader == HASH_CODE_RGEO) {
						...
						for (uVar4 = 0; uVar4 < (uVar3 & 0xffff); uVar4 = uVar4 + 1 & 0xffff) {
							if (*nextSubSection != 0) {
								/* Store a pointer to the next sub section */
								*(int*)(pcVar5 + *nextSubSection) = (int)STORE_POINTER(pcVar5 + (int)*(char**)(pcVar5 + *nextSubSection));
							}
...
void ed3DPrepareG2D(ed_g2d_manager* pManager, ulong mode)
{
	...
	if ((pManager != (ed_g2d_manager*)0x0) && ((pManager->pFileBuffer->flags & 1) == 0)) {
		...
		for (pBuffer = edChunckGetFirst(texHeaderStart + 1, (char*)texHeaderStart + texHeaderSize); pBuffer != (ed_Chunck*)0x0;
			...
			if (pBuffer->hash == HASH_CODE_LAYA) {
...
void ed3DHierarchyCopyHashCode(ed_g3d_manager* pMeshInfo)
{
	...
	for (uVar2 = 0; uVar2 < (uVar1 & 0xffff) - 1; uVar2 = uVar2 + 1) {
		ed_Chunck* p = (ed_Chunck*)LOAD_POINTER(*(pMVar3 + 1));
...
edNODE* ed3DHierarchyAddNode(edLIST* pList, ed_3d_hierarchy_node* pHierNode, edNODE* pNode, ed_g3d_hierarchy* pSourceHierarchy, ed_3d_hierarchy* p3DB)
{
	...
	if (hashA == hashB) {
		ED3D_LOG(LogLevel::Verbose, "ed3DRenderSonHierarchy {}", pSourceHierarchy->hash.ToString());
	}
	...
	// This is definitely not a data pointer, instead looks like a bunch of flags in ed_g3d_hierarchy, but in 
	// ed_3d_hierarchy it is a pointer to frame data for the shadow. Either way it is reset below so lets just cast it here.
	(pNewHierarchyNode->base).pShadowAnimMatrix = reinterpret_cast<edF32MATRIX4*>(pSourceHierarchy->pShadowAnimMatrix);
	...
	(pNewHierarchyNode->base).pHierarchySetup = (ed_3d_hierarchy_setup*)LOAD_POINTER(pSourceHierarchy->pHierarchySetup);
	(pNewHierarchyNode->base).pMatrixPkt = (edpkt_data*)LOAD_POINTER(pSourceHierarchy->pMatrixPkt);
	(pNewHierarchyNode->base).pAnimMatrix = (edF32MATRIX4*)LOAD_POINTER(pSourceHierarchy->pAnimMatrix);
...
#if 1 // validation
			ed_hash_code* pObjHash = LOAD_POINTER_CAST(ed_hash_code*, pDestLod->pObj);
			ed_Chunck* pObjChunk = LOAD_POINTER_CAST(ed_Chunck*, pObjHash->pData);
			ed_g3d_object* pObj = reinterpret_cast<ed_g3d_object*>(pObjChunk + 1);
#endif
		}
		else {
...
edNODE* RecursiveAddToHierarchy(
	...
	for (uint i = 0; i < totalElements; ++i) {
		ed_Chunck* pChunk = LOAD_POINTER_CAST(ed_Chunck*, pHashCode[i].pData);

		ed_g3d_hierarchy* pNewHier = (ed_g3d_hierarchy*)(pChunk + 1);

		ed_Chunck* pNewHierNode = LOAD_POINTER_CAST(ed_Chunck*, pNewHier->pLinkTransformData);
...
void ed3DHierarchyAddToListRecursive(edLIST* pList, ed_3d_hierarchy_node* pHierNode, edNODE* pNode, edNODE* pMeshTransformParent, ed_hash_code* pHashCode, ed_Chunck* peVar7)
{
	...
	ed_Chunck* iVar1 = (ed_Chunck*)LOAD_POINTER(pHashCode->pData);

	ed_g3d_hierarchy* pNewHier = (ed_g3d_hierarchy*)(iVar1 + 1);

	ed_3d_hierarchy_node* pNewHierNode = (ed_3d_hierarchy_node*)LOAD_POINTER(pNewHier->pLinkTransformData);
...
{
	ed_Chunck* pHierarchyChunk = (ed_Chunck*)0x0;
	ed_Chunck* pHALL = pMeshInfo->HALL;
	ed_Chunck* pHASH = pHALL + 1;
	ed_hash_code* pHashCodes = reinterpret_cast<ed_hash_code*>(pHASH + 1);

	uint nbChunks = edChunckGetNb(pHASH, reinterpret_cast<char*>(pHALL) + pHALL->size);
	nbChunks = (nbChunks & 0xffff) - 1;

	if (szString == (char*)0x0) {
		int subMeshIndex = -1;
		ed_hash_code* pCurHashCode = pHashCodes;

		// Loop through all the hash codes (should be one per chunk) until we get to the source chunk. Maybe looking for LOD0?
		for (uint curChunkIndex = 0; curChunkIndex < nbChunks; curChunkIndex = curChunkIndex + 1) {
			ed_Chunck* pHierChunk = LOAD_POINTER_CAST(ed_Chunck*, pCurHashCode->pData);
			ed_g3d_hierarchy* pHier = reinterpret_cast<ed_g3d_hierarchy*>(pHierChunk + 1);

			int subMeshCount = pHier->subMeshParentCount_0xac;
			if (subMeshIndex < subMeshCount) {
				pHierarchyChunk = LOAD_POINTER_CAST(ed_Chunck*, pCurHashCode->pData);
				subMeshIndex = subMeshCount;
			}

			pCurHashCode = pCurHashCode + 1;
		}
	}
	else {
		IMPLEMENTATION_GUARD(
	...
	if (pHierarchyChunk != (ed_Chunck*)0x0) {
		ed_g3d_hierarchy* const pSourceHier = reinterpret_cast<ed_g3d_hierarchy*>(pHierarchyChunk + 1);
		pRootNode = ed3DHierarchyAddNode(pList, pHierNode, pNode, pSourceHier, (ed_3d_hierarchy*)0x0);

		RecursiveAddToHierarchy(pList, pHierNode, pNode, pMeshInfo, pHierarchyChunk, pHashCodes, nbChunks, pRootNode);
	...
	ed_hash_code* pHashCode, uint count)
{
	...
	for (currentIndexA = 0; currentIndexA < count; currentIndexA = currentIndexA + 1) {
		pHierChunkA = LOAD_POINTER_CAST(ed_Chunck*, pHashCodeA->pData);
		ed_g3d_hierarchy* pHierarchy = (ed_g3d_hierarchy*)(pHierChunkA + 1);
		ed_3d_hierarchy_node* pHierarchyNode = LOAD_POINTER_CAST(ed_3d_hierarchy_node*, pHierarchy->pLinkTransformData);
		...
		if (pHierarchyNode == (ed_3d_hierarchy_node*)pChunck) {
			...
			for (currentIndexB = 0; currentIndexB < count; currentIndexB = currentIndexB + 1) {
				pHierChunkB = LOAD_POINTER_CAST(ed_Chunck*, pHashCodeB->pData);
				pHierarchy = (ed_g3d_hierarchy*)(pHierChunkB + 1);
				pHierarchyNode = LOAD_POINTER_CAST(ed_3d_hierarchy_node*, pHierarchy->pLinkTransformData);
				...
				if (pHierarchyNode == (ed_3d_hierarchy_node*)pHierChunkA) {
					...
					for (local_70 = 0; local_70 < count; local_70 = local_70 + 1) {
						pcVar2 = (char*)LOAD_POINTER(local_90->pData);
						if (*(char**)(pcVar2 + 0xa0) == (char*)pHierChunkB) {
							...
							for (local_c0 = 0; local_c0 < count; local_c0 = local_c0 + 1) {
								pcVar3 = (char*)LOAD_POINTER(local_e0->pData);
								if (*(char**)(pcVar3 + 0xa0) == pcVar2) {
									...
									for (local_110 = 0; local_110 < count; local_110 = local_110 + 1) {
										pcVar4 = (char*)LOAD_POINTER(local_130->pData);
													...
													for (local_1b0 = 0; local_1b0 < count; local_1b0 = local_1b0 + 1) {
														...
														if ((peVar6->base).pHierarchySetup == peVar5) {
															...
															for (local_200 = 0; local_200 < count; local_200 = local_200 + 1) {
																...
																pHierarchyNode = (ed_3d_hierarchy_node*)LOAD_POINTER(pHierarchy->pLinkTransformData);
...
ed_Chunck* ed3DHierarchyNodeGetSkeletonChunck(edNODE* pMeshTransformParent, bool bGetFromHierarc)
{
	...
	if ((pMeshTransformData->lodCount != 0) &&
		...
		pLodHash = (ed_hash_code*)LOAD_POINTER(pLod[desiredLod].pObj);

		if (pLodHash == (ed_hash_code*)0x0) {
			pLodHash = (ed_hash_code*)LOAD_POINTER(pLod[0].pObj);

			ED3D_LOG(LogLevel::VeryVerbose, "ed3DHierarchyNodeGetSkeletonChunck could not find L.O.D defaulting to 0");
		}

		// Look for .obj
		if ((pLodHash != (ed_hash_code*)0x0) && (pObjChunck = (ed_Chunck*)LOAD_POINTER(pLodHash->pData), pObjChunck->hash == 0x2e4a424f /*OBJ*/)) {
...
void CHierarchyAnm::Install(MeshData_ANHR* pInANHR, int length, ed_g3d_manager* pMeshInfo, ed_3D_Scene* pStaticMeshMaster)
{
	...
	if (pInANHR == (MeshData_ANHR*)0x0) {
		...
		for (uVar9 = 0; uVar11 = 0, uVar9 < fileDatEntryCount; uVar9 = uVar9 + 1) {
			aFileData[uVar9] = (int)STORE_POINTER((char*)pInANHR + aFileData[uVar9]);
		}
		...
		for (; uVar11 < otherEntryCount; uVar11 = uVar11 + 1) {
			...
			pBaseIndexes[uVar11] = (int)STORE_POINTER(pANHR_Internal);
			...
			if (iVar5 == 0) {
				...
				ed_Chunck* pChunck = (ed_Chunck*)LOAD_POINTER(iVar5);
				pNewNode = ed3DHierarchyAddNode(pList, pHierNode, pParentNode, (ed_g3d_hierarchy*)(pChunck + 1), (ed_3d_hierarchy*)0x0);
				ed3DHierarchyAddSonsToList(pList, pHierNode, pParentNode, (ed_Chunck*)LOAD_POINTER(iVar5), pNewNode, (ed_hash_code*)(pcVar2 + 2), (fileDatEntryCount & 0xffff) - 1);
				ed3DHierarchyRefreshSonNumbers(pNewNode, &sStack2);
			}

			pANHR_Internal->pHierNode = STORE_POINTER(pNewNode);
			if (pNewNode == (edNODE*)0x0) {
				pANHR_Internal->pHierNodeData = 0x0;
				pANHR_Internal->pHierAnimStream = 0x0;
			}
			else {
				pANHR_Internal->pHierNodeData = STORE_POINTER(pNewNode->pData);
				...
				if ((pANHR_Internal->flags & 4) != 0) {
					ed3DHierarchySetSetup(LOAD_POINTER_CAST(ed_3d_hierarchy*, pANHR_Internal->pHierNodeData), &ed_3d_hierarchy_setup_0040f070);
				}

				pHierAnimStream = (S_HIERANM_ANIM*)LOAD_POINTER(aFileData[(int)pANHR_Internal->pHierAnimStream]);

				UpdateMatrix(pANHR_Internal->field_0x28, &((ed_3d_hierarchy_node*)LOAD_POINTER(pANHR_Internal->pHierNodeData))->base.transformA, pHierAnimStream, pANHR_Internal->flags & 1);
				...
				if (((pHierAnimStream->field_0x0 < 2) && (pHierAnimStream->field_0x4 < 2)) && (pHierAnimStream->field_0x8 < 2)) {
					...
					pANHR_Internal->pHierAnimStream = STORE_POINTER(pHierAnimStream);
				}
				...
				if (pSkeletonChunck == (ed_Chunck*)0x0) {
					...
					pANHR_Internal->pObbFloat.pObb_Internal = STORE_POINTER(pSkeletonChunck + 1);

					local_8 = edAnmSkeleton((edANM_SKELETON*)LOAD_POINTER(pANHR_Internal->pObbFloat.pObb_Internal));
					...
					for (uVar9 = 0; uVar9 < fileDatEntryCount; uVar9 = uVar9 + 1) {
						...
						pNext->chunk = STORE_POINTER(LOAD_POINTER(aFileData[pNext->chunk]));
						pNext = pNext + 1;
					}
...
void CHierarchyAnm::Manage(float param_1, float param_2, ed_3D_Scene* pScene, int param_5)
{
	...
	if (this->pThis != (MeshData_ANHR*)0x0) {
		...
		for (uVar12 = 0; uVar12 < uVar2; uVar12 = uVar12 + 1) {
			...
			ANHR_Internal* peVar3 = (ANHR_Internal*)LOAD_POINTER(pNext[uVar12]);
			pNode = (edNODE*)LOAD_POINTER(peVar3->pHierNode);
			if (pNode != (edNODE*)0x0) {
				local_4 = 0.0f;
				...
				ed_3d_hierarchy_node* pHierNodeData = (ed_3d_hierarchy_node*)LOAD_POINTER(peVar3->pHierNodeData);
				...
				if (bVar6) {
					...
					edAnmSkeleton skeleton = edAnmSkeleton((edANM_SKELETON*)LOAD_POINTER(peVar3->pObbFloat.pObb_Internal));
...
edNODE* ed3DHierarchyAddToSceneByHashcode(ed_3D_Scene* pStaticMeshMaster, ed_g3d_manager* pMeshInfo, ulong hash)
{
	...
	if (pcVar2 != (ed_hash_code*)0x0) {
		pChunck = (ed_Chunck*)LOAD_POINTER(pcVar2->pData);
	}
...
void ed3DLinkStripToViewport(ed_3d_strip* pStrip, edF32MATRIX4* pMatrix, ed_hash_code* pHash, edpkt_data* pPktToLink)
{
	...
	pScratchRenderInfo = gRender_info_SPR;
	...
	RenderInfo_00424f80.pPkt = pPktToLink;

	gRender_info_SPR->pSharedMeshTransform = pMatrix;
	pScratchRenderInfo->pMeshTransformMatrix = RenderInfo_00424f80.pMeshTransformMatrix;
	pScratchRenderInfo->boundingSphereTestResult = RenderInfo_00424f80.boundingSphereTestResult;
	pScratchRenderInfo->pHierarchySetup = RenderInfo_00424f80.pHierarchySetup;
	pScratchRenderInfo->flags = RenderInfo_00424f80.flags;
	pScratchRenderInfo->biggerScale = RenderInfo_00424f80.biggerScale;
	pScratchRenderInfo->pPkt = RenderInfo_00424f80.pPkt;
	pScratchRenderInfo->pMeshTransformData = RenderInfo_00424f80.pMeshTransformData;
...
{
	ed_Chunck* pObj = (ed_Chunck*)LOAD_POINTER(pMeshHeader->pData);
...
ed_g3d_object* ed3DHierarchyGetObject(ed_3d_hierarchy* pHier)
{
	...
	if (pHier->lodCount != 0) {
		...
		if (peVar2 == (ed3DLod*)0x0) {
			...
			if (sVar1 == 3) {
				piVar3 = ed3DObjectGetFromHashCode((ed_hash_code*)LOAD_POINTER(peVar2->pObj));
			}
			else {
				if ((sVar1 != 2) && (sVar1 != 1)) {
					if (sVar1 == 0) {
...
ed_g3d_hierarchy* ed3DG3DHierarchyGetFromHashcode(ed_g3d_manager* pG3d, ulong hash)
{
	...
	if ((pcVar1 == (ed_hash_code*)0x0) || (pcVar1->pData == 0x0)) {
		...
		peVar1 = (ed_g3d_hierarchy*)((char*)LOAD_POINTER(pcVar1->pData) + 0x10);
	}
...
void ed3DScenePopCluster(ed_3D_Scene* pScene, ed_g3d_manager* pMeshInfo)
{
	...
	peVar1 = (pScene->meshClusterList).pPrev;
	peVar5 = peVar1;
	do {
		peVar4 = peVar5;
		peVar5 = peVar1;
	...
	if ((peVar4 == peVar5) && (peVar4 == (pScene->meshClusterList).pPrev)) {
...
bool ed3DComputeSceneCoordinate(edF32VECTOR2* pOutScreenCoord, edF32VECTOR4* pPosition, ed_3D_Scene* pScene)
{
	float fov;
	float ratio;
	edF32VECTOR4 screenCoordsCamera;
	edF32MATRIX4 eStack128;
	edF32MATRIX4 flippedWorldToCamera;
	edFCamera* pCamera;

	edF32Matrix4SetIdentityHard(&eStack128);
	eStack128.cc = -1.0f;
	pCamera = pScene->pCamera;
	fov = edFCameraGetFov(pCamera);
	ratio = edFCameraGetRatio(pCamera);
	edF32Matrix4MulF32Matrix4Hard(&flippedWorldToCamera, &pScene->pCamera->worldToCamera, &eStack128);
	edF32Matrix4MulF32Vector4Hard(&screenCoordsCamera, &flippedWorldToCamera, pPosition);

	if (0.0f < screenCoordsCamera.z) {
		pOutScreenCoord->x = (screenCoordsCamera.x / ratio) / (screenCoordsCamera.z * fov);
		pOutScreenCoord->y = screenCoordsCamera.y / (screenCoordsCamera.z * fov);
	}

	return 0.0f < screenCoordsCamera.z;
}
...
bool ed3DComputeScreenCoordinate(float z, edF32VECTOR4* pWorldPosition, edF32VECTOR2* pScreenCoordinate, ed_3D_Scene* pScene)
{
	...
	if (0.0f < z) {
		...
		local_90.y = pScreenCoordinate->y * z * fVar1;
		local_90.w = 1.0f;
		local_90.x = pScreenCoordinate->x * fVar2 * z * fVar1;
		local_90.z = z;
