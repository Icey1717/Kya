...
{
	float perpDistance;
	edF32VECTOR4 planeToPoint;

	edF32Vector4SubHard(&planeToPoint, pPointPosition, pPlanePoint);

	// Project vector onto plane normal.
	perpDistance = edF32Vector4DotProductHard(&planeToPoint, pPlaneNormal);
...
void edF32Matrix4MulF32Vector4Hard(edF32VECTOR4* v0, edF32MATRIX4* m0, edF32VECTOR4* v1)
{
	...
	dc = m0->dc;
	dd = m0->dd;
	v0->x = m0->aa * v1X + m0->ba * v1Y + m0->ca * v1Z + m0->da * v1W;
	v0->y = ab * v1X + bb * v1Y + cb * v1Z + db * v1W;
	v0->z = ac * v1X + bc * v1Y + cc * v1Z + dc * v1W;
	v0->w = ad * v1X + bd * v1Y + cd * v1Z + dd * v1W;
	return;
}
