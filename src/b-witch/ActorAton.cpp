...
#define ATON_ESCAPE_STATE_WAIT_JUMP_CALL_NEAR			0x19
...
void CActorAton::BehaviourAtonEscape_Manage()
{
	...
	if (this->trailMaterialId != -1) {
		if ((GetStateFlags(this->actorState) & 0x1000) == 0) {
			...
			if (fVar23 < 30.0f) {
				fVar23 = fVar23 / 30.0f;
				fVar27 = 1.0f - fVar23;
				(this->fxTailA).previousColor = (this->fxTailA).color;
				fVar24 = fVar27 * 128.0f;
				(this->fxTailA).color.r = 0x80;
				uVar22 = EncodeFloat(fVar24);
				fVar27 = fVar27 * 128.0f;
				(this->fxTailA).color.g = uVar22;
				uVar22 = EncodeFloat(fVar27);
				fVar23 = fVar23 * 128.0f;
				(this->fxTailA).color.b = uVar22;
				this->fxTailA.color.a = EncodeFloat(fVar23);
			}
			else {
				(this->fxTailA).previousColor = (this->fxTailA).color;
				(this->fxTailA).color.r = 0x80;
				(this->fxTailA).color.g = 0;
				(this->fxTailA).color.b = 0;
				(this->fxTailA).color.a = 0x80;
			}

			this->fxTailA.Manage(&eStack256, &eStack272, 0);
		}
	}
	...
	if (this->trailMaterialId != -1) {
		if ((GetStateFlags(this->actorState) & 0x1000) == 0) {
			...
			SV_GetBoneWorldPosition(uVar12, &eStack288);
			SV_GetBoneWorldPosition(uVar1, &eStack304);

			fVar23 = this->dynamic.linearAcceleration;
			if (fVar23 < 30.0f) {
				fVar23 = fVar23 / 30.0f;
				fVar27 = 1.0f - fVar23;
				(this->fxTailB).previousColor = (this->fxTailB).color;
				fVar24 = fVar27 * 128.0f;
				(this->fxTailB).color.r = 0x80;
				uVar22 = EncodeFloat(fVar24);
				fVar27 = fVar27 * 128.0f;
				(this->fxTailB).color.g = uVar22;
				uVar22 = EncodeFloat(fVar27);
				fVar23 = fVar23 * 128.0f;
				(this->fxTailB).color.b = uVar22;
				this->fxTailB.color.a = EncodeFloat(fVar23);
			}
			else {
				(this->fxTailB).previousColor = (this->fxTailB).color;
				(this->fxTailB).color.r = 0x80;
				(this->fxTailB).color.g = 0;
				(this->fxTailB).color.b = 0;
				(this->fxTailB).color.a = 0x80;
			}
			this->fxTailB.Manage(&eStack288, &eStack304, 0);
		}
	}
	...
	LAB_00384948:
		...
		break;
	case ATON_ESCAPE_STATE_WAIT_JUMP_CALL_NEAR:
		StateAtonPathWaitJumpCallNear();
		break;
...
void CActorAton::StateAtonPathWaitJumpCallNear()
{
	int iVar1;
	CAnimation* pCVar2;
	edAnmLayer* peVar3;
	...
	ManageDyn(4.0f, 0x2003b, (CActorsTable*)0x0);
	pCVar9 = CActorHero::_gThis;
	local_50 = pCVar9->currentLocation - this->currentLocation;
	fVar14 = sqrtf(local_50.x * local_50.x + local_50.y * local_50.y + local_50.z * local_50.z);
...
CBehaviourAddOnAton::CBehaviourAddOnAton()
{
	...
	this->aAtonSubObjs = (CAddOnSubObjAton*)0x0;
	this->nbOtherSubObjs = 0;
	this->aOtherSubObjs = (CAddOnSubObj*)0x0;
}
...
void CBehaviourAddOnAton::Create(ByteCode* pByteCode)
{
	...
	this->nbOtherSubObjs = pByteCode->GetS32();

	uVar1 = this->nbOtherSubObjs;
	if (uVar1 != 0) {
		this->aOtherSubObjs = new CAddOnSubObj[this->nbOtherSubObjs];
		iVar9 = 0;
		if (0 < this->nbOtherSubObjs) {
			do {
				this->aOtherSubObjs[iVar9].Create(pByteCode);
				iVar9 = iVar9 + 1;
	...
	if (uVar1 != 0) {
		this->aAtonSubObjs = new CAddOnSubObjAton[this->nbAtonSubObjs];
		iVar9 = 0;
		if (0 < this->nbAtonSubObjs) {
			do {
				this->aAtonSubObjs[iVar9].Create(pByteCode);
				iVar9 = iVar9 + 1;
...
void CBehaviourAddOnAton::Init(CActor* pOwner)
{
	this->pOwner = pOwner;
	this->pSubObj = 0;
	this->field_0xc = 0;
...
void CBehaviourAddOnAton::Manage()
{
	...
	this->field_0x28 = this->field_0x28 + GetTimer()->cutsceneDeltaTime;
...
LAB_003e2fa0:
	if (!bVar1) {
		...
		if (0 < this->nbOtherSubObjs) {
			do {
				pCurSubObj = this->aOtherSubObjs + iVar4;
				if (pCurSubObj != (CAddOnSubObj*)0x0) {
					fVar5 = pCurSubObj->field_0x14 + GetTimer()->cutsceneDeltaTime;
					pCurSubObj->field_0x14 = fVar5;

					if ((1.0f <= fVar5) && (pCurSubObj->pCinematic != (CCinematic*)0x0)) {
						pCurSubObj->SetCinematic((CCinematic*)0x0);
					}
				}
		...
		if (0 < this->nbAtonSubObjs) {
			iVar3 = 0;
			do {
				pCurSubObj = this->aAtonSubObjs + iVar4;
				if (pCurSubObj != (CAddOnSubObj*)0x0) {
					fVar5 = pCurSubObj->field_0x14 + GetTimer()->cutsceneDeltaTime;
					pCurSubObj->field_0x14 = fVar5;
					if ((1.0f <= fVar5) && (pCurSubObj->pCinematic != (CCinematic*)0x0)) {
...
void CAddOnSubObjAton::Create(ByteCode* pByteCode)
{
	this->field_0x18 = pByteCode->GetU32();

	CAddOnSubObj::Create(pByteCode);
